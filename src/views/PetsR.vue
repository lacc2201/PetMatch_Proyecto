    ...
    <div>
        ...
        <section class="reviews-section">
            ...
            <div style="position: fixed; bottom: 20px; right: 20px;">
                <button @click="openModal" class="add-mascota-btn">
                    🐾
                </button>
            </div>

            <!-- Modal overlay -->
            <div v-if="modalVisible" id="modal-overlay" class="modal-overlay"></div>

            <!-- Modal content para agregar mascota -->
            <div v-if="modalVisible" id="modal" class="modal-content-wrapper">
                <button @click="closeModal" class="close-btn">&times;</button>
                <h2>Agregar Nueva Mascota</h2>
                <form @submit.prevent="agregarMascota">
                    <label for="nombre">Nombre de la Mascota:</label>
                    <input type="text" v-model="nuevaMascota.nombre" required class="input-field">

                    <label for="tipo">Tipo:</label>
                    <input type="text" v-model="nuevaMascota.tipo" required class="input-field">

                    <label for="raza">Raza:</label>
                    <input type="text" v-model="nuevaMascota.raza" required class="input-field">

                    <label for="descripcion">Descripción:</label>
                    <textarea v-model="nuevaMascota.descripcion" required class="input-field"></textarea>

                    <label for="foto">Foto de la Mascota:</label>
                    <input type="file" @change="mostrarVistaPrevia" accept="image/*" required class="input-field">
                    <img v-if="fotoPreview" :src="fotoPreview" alt="Vista previa" class="preview-img">

                    <img v-if="fotoPreview" :src="fotoPreview" alt="Vista previa" class="preview-img">

                    <button type="submit" class="submit-btn">Agregar Mascota</button>
                </form>
            </div>
        </section>
        ...
        <div class="">
            <div class="filter-buttons">
                ...
                <button @click="showSection('sold-mascotas')">Adoptadas</button>
            </div>

            <!-- Sección Todas las Mascotas -->
            <div id="all-mascotas" v-if="activeSection === 'all-mascotas'">
                <h2>Todas las Mascotas</h2>
                <div class="mascotas-lista">
                    <div v-for="mascota in todasLasMascotas" :key="mascota.id">
                        <div class="mascota">
                            <img :src="mascota.foto_url" :alt="mascota.nombre" class="mascota-img" />
                            ...
                            <button v-if="mascota.estado !== 'adoptada'" @click="marcarComoAdoptada(mascota)"
                                class="adopt-button">Marcar como Adoptada</button>
                            <button v-else class="adopted-button">Adoptada</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id="available-mascotas" v-if="activeSection === 'available-mascotas'">
                ...
                <div class="mascotas-lista">
                    <div v-for="mascota in mascotasDisponibles" :key="mascota.id">
                        <div class="mascota">
                            ...
                            <p><strong>Estado:</strong> {{ mascota.estado }}</p>
                            <p><strong>Fecha de Ingreso:</strong> {{ mascota.fecha_ingreso }}</p>
                            <button @click="marcarComoAdoptada(mascota)" class="adopt-button">Marcar como
                                Adoptada</button>
                        </div>
                    </div>
                </div>
            ...
            <!-- Sección Mascotas Adoptadas -->
            <div id="sold-mascotas" v-if="activeSection === 'sold-mascotas'">
...
<script>
import { addDoc, collection, doc, onSnapshot, updateDoc } from 'firebase/firestore'; // Añadí doc, updateDoc y addDoc
...
export default {
    ...
    data() {
        return {
            ...
            fotoPreview: null,
            activeSection: "all-mascotas", // Sección activa
            todasLasMascotas: [], // Array con todas las mascotas
            mascotasDisponibles: [], // Array con mascotas disponibles
            mascotasVendidas: [], // Array con mascotas adoptadas
        };
    },
    methods: {
        // Método para escuchar cambios en tiempo real de todas las mascotas
        obtenerMascotas() {
            const mascotasRef = collection(db, 'mascotas'); // Aquí 'mascotas' es la colección
            // Usamos onSnapshot para escuchar cambios en tiempo real
            onSnapshot(mascotasRef, (snapshot) => {
                this.todasLasMascotas = [];
                snapshot.forEach((doc) => {
                    const mascota = doc.data();
                    this.todasLasMascotas.push({
                        id: doc.id,
                        ...mascota,
                    });
                });

                // Filtrar las mascotas disponibles
                this.mascotasDisponibles = this.todasLasMascotas.filter(mascota => mascota.estado === 'disponible');

                // Filtrar las mascotas adoptadas
                this.mascotasVendidas = this.todasLasMascotas.filter(mascota => mascota.estado === 'adoptada');
            });
        },

        // Función para cambiar la sección activa y filtrar las mascotas
        showSection(section) {
            this.activeSection = section;

            // Filtrar según la sección activa
            if (section === 'all-mascotas') {
                this.mascotasFiltradas = this.todasLasMascotas;  // Todas las mascotas
            } else if (section === 'disponibles') {
                this.mascotasFiltradas = this.mascotasDisponibles;  // Solo las disponibles
            } else if (section === 'adoptadas') {
                this.mascotasFiltradas = this.mascotasVendidas;  // Solo las adoptadas
            }
        },

        // Método para marcar una mascota como adoptada
        marcarComoAdoptada(mascota) {
            const mascotaRef = doc(db, 'mascotas', mascota.id); // Asegúrate de usar `doc` aquí
            updateDoc(mascotaRef, { estado: 'adoptada' }).then(() => {
                alert(`La mascota ${mascota.nombre} ha sido adoptada.`);
            }).catch((error) => {
                console.error("Error al marcar la mascota como adoptada: ", error);
            });
        },

        // Función para abrir el modal de agregar nueva mascota
        openModal() {
        ...
        // Previsualización de la foto de la mascota antes de subirla
        ...
        agregarMascota() {
            // Asumiendo que ya estás subiendo la foto a Firebase Storage y obteniendo la URL
            const foto_url = this.fotoPreview;  // O la URL de Firebase Storage
            const nuevaMascotaRef = collection(db, 'mascotas');
            addDoc(nuevaMascotaRef, {
                ...this.nuevaMascota,
                foto_url,
                estado: 'disponible', // Al agregarla, la dejamos como disponible
    ...
    mounted() {
        // Llamamos a obtenerMascotas al montar el componente para cargar los datos al inicio
...
/* Estilo para la tarjeta de mascota */
.mascota {
    display: flex;
    /* Usamos Flexbox */
    align-items: center;
...
.agregar-mascota-btn {
    position: fixed;
    bottom: 40px;
    /* Ajustar el espacio sobre el footer */
    right: 20px;
    background-color: #f5c20b;
    color: white;
    width: 50px;
...
/* Mostrar el formulario cuando tenga la clase "activo" */
...
form.formulario-agregar h3 {
    text-align: center;
    color: #000000;
}

form.formulario-agregar label {
    margin-bottom: 5px;
    /* Espaciado entre label y input/select */
    color: #000000;
    font-weight: normal;
    /* Se quita la negrita */
    display: block;
    /* Asegura que cada label esté en su propia línea */
}

form.formulario-agregar input,
form.formulario-agregar select {
    padding: 10px;
    margin-bottom: 15px;
...
form.formulario-agregar button {
    padding: 10px;
...
form.formulario-agregar button:hover {
...
.contactar-btn {
    ...
    padding: 12px 25px;
    /* Mayor espacio dentro del botón */
    border: none;
    border-radius: 8px;
    /* Bordes más redondeados */
    cursor: pointer;
...
.mascota {
    ...
    /* Usamos Flexbox */
    flex-direction: column;
    /* Alineación vertical para que los elementos estén apilados */
    align-items: flex-start;
    /* Alineamos los elementos al inicio */
    justify-content: flex-start;
    /* Alineamos el contenido hacia el principio */
    background-color: #ffffff;
    padding: 15px;
    /* Reduce el padding para hacer la tarjeta más pequeña */
    border-radius: 12px;
    /* Bordes redondeados */
    ...
    height: 430px;
    /* Altura ajustada para la tarjeta */
    width: 260px;
    /* Ancho reducido */
    overflow: hidden;
    /* Asegura que nada sobresalga de la tarjeta */
}
...
.mascota img {
    width: 90%;
    /* Hace que la imagen ocupe el 90% del ancho de la tarjeta */
    height: 180px;
    ...
    border-radius: 10px;
    margin: 0 auto 10px;
    /* Centra la imagen y agrega un margen inferior */
    display: block;
    /* Asegura que la imagen sea un bloque y pueda centrarse */
}
...
.mascota .contactar {
    align-self: flex-end;
    /* Alinea el botón hacia la parte inferior */
