<template>
    <div>
      ...
      <main>
        <div class="contenedor__todo">
          ...
          <div class="contenedor__login-register">
            <!-- Login -->
            <form id="login-form" class="formulario__login" v-show="mostrarLoginForm" @submit.prevent="validarFormularioLogin">
              <h2>🐾 Iniciar Sesión 🐾</h2>
              <input type="email" id="login-email" placeholder="Correo Electrónico" v-model="loginEmail" required />
              <div class="password-container">
                <input type="password" id="login-password" placeholder="Contraseña" v-model="loginPassword" required />
                <button type="button" class="toggle-password" @click="togglePassword('login-password')">👁️</button>
              </div>
              <button type="submit">Entrar</button>
            </form>
  
            <!-- Register -->
            <form class="formulario__register" v-show="mostrarRegisterForm" @submit.prevent="validarFormularioRegistro">
              <h2>🐶 Regístrate 🐱</h2>
              <input type="text" id="register-nombre" placeholder="Nombre Completo" v-model="registerNombre" required />
              <input type="email" id="register-email" placeholder="Correo Electrónico" v-model="registerEmail" required />
              <input type="text" id="register-direccion" placeholder="Dirección" v-model="registerDireccion" required />
              <input type="tel" id="register-telefono" placeholder="Teléfono Celular" v-model="registerTelefono" required />
              <div class="password-container">
                <input type="password" id="register-password" placeholder="Contraseña" v-model="registerPassword" required />
                <button type="button" class="toggle-password" @click="togglePassword('register-password')">👁️</button>
              </div>
      ...
      <footer>
        ...
        <div class="footer-bottom">
          ...
          <p>
            Con amor y dedicación | <a href="#">Política de Privacidad</a> | <a href="#">Términos y Condiciones</a>
          </p>
        </div>
      </footer>
    </div>
  ...
  import { doc, setDoc, getDoc } from "firebase/firestore"; // Firestore para guardar y leer datos
    ...
    methods: {
      ...
      async validarFormularioLogin() {
        try {
          ...
          if (docSnap.exists()) {
            ...
            if (userRole === 'cliente') {
              alert('Inicio de sesión exitoso. Redirigiendo a la vista de Cliente.');
              window.location.href = '/HomeU'; // Cambiar a la ruta adecuada
            } else if (userRole === 'vendedor') {
              alert('Inicio de sesión exitoso. Redirigiendo a la vista de Vendedor.');
              window.location.href = '/HomeV'; // Cambiar a la ruta adecuada
            } else {
              alert('Rol no válido.');
            }
          } else {
      ...
      async validarFormularioRegistro() {
        try {
          ...
          // Guardar información del usuario en Firestore, asignando rol de 'Cliente'
