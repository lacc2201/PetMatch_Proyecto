<template>
    <div>
        ...
        <header>
            ...
            <div class="header-title">Panel de Admin</div>
            <nav>
                <router-link to="/AdminH" class="nav-link">PANEL PRINCIPAL</router-link>
        ...
        <section class="reviews-section">
            <div class="title">Estado de Mascotas</div>
            <br /><br />
            <div class="filter-buttons">
                <button class="filter-button" @click="filterPets('available')">Disponibles</button>
                <button class="filter-button" @click="filterPets('in-process')">En Proceso</button>
                <button class="filter-button" @click="filterPets('adopted')">Adoptadas</button>
            </div>
        </section>
        ...
        <div class="container">
            <ul class="pet-list">
                <li class="pet-item" v-for="pet in filteredPets" :key="pet.id" :data-status="pet.estado">
                    <img class="pet-img" :src="pet.foto_url || 'https://via.placeholder.com/150'"
                        :alt="pet.nombre_mas" />
                    <div class="pet-details">
                        <span class="pet-name">{{ pet.nombre_mas }}</span>
                        <p class="pet-desc">{{ pet.descripcion }}</p>
                    </div>
                    <select class="status-select" v-model="pet.estado" @change="updatePetStatus(pet)">
                        <option value="available">Disponible</option>
                        <option value="in-process">En Proceso</option>
                        <option value="adopted">Adoptada</option>
                    </select>
                </li>
            </ul>
        </div>
    ...
    methods: {
        ...
        async loadPets() {
            try {
                ...
                this.pets = petDocs.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
                console.log("Mascotas cargadas:", this.pets); // Verifica la estructura de los datos
                this.filteredPets = this.pets;
            } catch (error) {
                console.error("Error cargando las mascotas:", error);
                alert("No se pudieron cargar las mascotas. Revisa la conexión a Firebase.");
            }
        },
        ...
        filterPets(status) {
            ...
            if (status === "all") {
                ...
                this.filteredPets = this.pets.filter((pet) => pet.estado === status);
            }
        },

        // Actualizar el estado de una mascota en Firestore
        async updatePetStatus(pet) {
            try {
                const petRef = doc(db, "mascotas", pet.id);
                await updateDoc(petRef, { estado: pet.estado });
                alert(`El estado de "${pet.nombre_mas}" ha sido actualizado a "${pet.estado}"`);
            } catch (error) {
...
.action-card {
    ...
    /* Color de texto */
    padding: 20px;
    /* Espacio interno ajustado (reducido) */
    border-radius: 10px;
    /* Bordes ligeramente más redondeados */
    text-align: center;
    /* Centrar texto */
    width: 300px;
    /* Aumentar ancho de las tarjetas */
    height: 70px;
    /* Añadir altura para hacerlas más cuadradas */
    transition: all 0.3s ease;
    /* Transiciones suaves */
    text-decoration: none;
    /* Sin subrayado */
...
.action-card i {
    ...
    margin-bottom: 5px;
    /* Reducir espacio debajo del ícono */
    display: block;
