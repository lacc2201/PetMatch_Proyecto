<template>
    <div>
            ...
            <tbody>
                <tr v-for="user in users" :key="user.id">
                    ...
                    <td class="action-buttons">
                        ...
                        <button class="status-btn" @click="toggleStatus(user)">{{ user.estado ? "Desactivar" : "Activar"
                            }}</button>
                    </td>
                </tr>
            </tbody>
        ...
        <div id="paginationControls" class="pagination">
            <button class="prevPage" :disabled="currentPage === 1">Anterior</button>
            <span class="pageNumbers">Página {{ currentPage }} de {{ totalPages }}</span>
            <button class="nextPage" :disabled="currentPage === totalPages">Siguiente</button>
        </div>

        <div v-if="isEditModalOpen" class="modal">
            <div class="modal-content">
                <h3>Editar Usuario</h3>
                <input v-model="currentUser.nombre" placeholder="Nombre">
                <input v-model="currentUser.email" placeholder="Email">
                <input v-model="currentUser.rol" placeholder="Rol">
...
export default {
    ...
    data() {
        return {
            ...
            itemsPerPage: 10, // Usuarios por página
            isEditModalOpen: false, // Estado del modal de edición
            currentUser: null // Usuario en edición
        };
    },
    async mounted() {
    ...
    methods: {
        ...
        openEditModal(user) {
            ...
            this.isEditModalOpen = true; // Cambiar el estado del modal a abierto
        },
        closeEditModal() {
            this.isEditModalOpen = false; // Cierra el modal
        ...
        async updateUser(user) {
                try {
                    ...
                    await updateDoc(userRef, {
                        ...
                        email: user.email,
                        rol: user.rol,
                        estado: user.estado // Asegurarse de incluir estado
                    });
                    await this.fetchUsers(); // Refrescar lista de usuarios
                } catch (error) {
        ...
        async toggleStatus(user) {
                try {
                    const userRef = doc(db, "usuarios", user.id);
                    const newState = !user.estado; // Alternar el estado actual
                    await updateDoc(userRef, { estado: newState });
                    user.estado = newState; // Actualizar el estado localmente
                } catch (error) {
